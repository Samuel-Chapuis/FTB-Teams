...
public final class MinionAssignmentsTest {
	...
	public static void main(String[] args) {
		...
		UUID faction = UUID.randomUUID(), resident = UUID.randomUUID(), replacement = UUID.randomUUID();
		check(homes.claim(42, resident, faction), "first validation assigns the resident");
		check(!homes.claim(42, replacement, faction), "repeated validation must not duplicate a resident");
		check(!homes.claim(84, resident, faction), "one resident cannot occupy two beds");
		check(!homes.claim(42, replacement, UUID.randomUUID()), "another faction cannot replace the resident");
		...
		saved.forEach((home, entry) -> loaded.claim(home, entry.minion(), entry.faction()));
		...
		check(!loaded.claim(42, replacement, faction), "an unloaded resident still occupies its bed");
		...
		check(loaded.claim(42, replacement, faction), "later validation can replace a dead resident");
		...
		check(loaded.claim(84, resident, faction), "released resident no longer reserves another bed");
